# https://github.com/YouHoo0521/kaggle-march-madness-men-2019/blob/master/src/data/make_dataset.py

import pandas as pd
import numpy as np
import os

DATAFILES_BASEDIR = r'D:\Documents\PyCharmProjects\MarchMania21\Data'
...
def get_train_data_v1(season=None, detailed=False):
    ...
    RegularSeasonResults = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MRegularSeason{}Results.csv'.format(detail)))
    NCAATourneyResults = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MNCAATourney{}Results.csv'.format(detail)))
    NCAATourneySeeds = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MNCAATourneySeeds.csv'))
    TeamConferences = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MTeamConferences.csv'))
    Conferences = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'Conferences.csv'))
    Teams = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MTeams.csv'))
    ...
    SampleSubmissionStage1 = pd.read_csv(
        os.path.join(DATAFILES_BASEDIR, 'MSampleSubmissionStage1_2020.csv'))
    tourney2019 = SampleSubmissionStage1['ID'].str.split('_', expand=True).astype(int)
    ...
    if season:
        ...
        data = data.loc[(data.Season < 2020) & (data.Season > 2010)]  # filter season
    ##################################################
...
def get_boxscore_dataset_v1(season=None, detailed=False, final_prediction=False):
    ...
    '''
    if final_prediction:
        data_sub = pd.read_csv('MSampleSubmissionStage1_2020.csv')
    ...
    ##################################################
    RegularSeasonDetailedResults = pd.read_csv(os.path.join(DATAFILES_BASEDIR, 'MRegularSeasonDetailedResults.csv'))
    ##################################################
    # column processing
    ##################################################
    ...
    ##################################################
    RegularSeasonDetailedResultsStacked = pd.concat([RegularSeasonDetailedResults,
         RegularSeasonDetailedResults.rename(reverse_dict, axis=1)], sort=True).rename(rename_dict, axis=1)
